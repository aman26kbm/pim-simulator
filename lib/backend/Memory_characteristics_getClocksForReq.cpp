...
int MemoryCharacteristics::getClocksForReq(Request& req, string op, int levels)
{
    int clocks=0;
    int bits;
    string dtype;
    int mantissa;
    int exponent;
    std::vector<pimsim::PrecisionT::Precision> precision_list = req.precision_list;
    if (op == "add")
    {
        if (precision_list[0].isfloat)
            dtype = "float";
        else
            dtype = "int";
    ...
    else if (op == "add_cram_rf")
    {
        ...
        //         clocks = hTreeTile::getCycles(req, config) + req.precision_list[1].bits();//tree height + precition
        //     else if(config->_tile_interconnect == "bus")
        //         clocks = 1 + req.precision_list[1].bits();//only need 1 cycle to drive the bus, then precition cycles to store in cram rows.
        //     else if(config->_tile_interconnect == "ideal")
        //         clocks = req.precision_list[1].bits();//only need precition cycles to store in cram rows.
        // }
        //cycles to do actuall computation in array
        if (precision_list[0].isfloat)
            dtype = "float";
        else
            dtype = "int";
    ...
    else if (op == "mul_cram_rf")
    {
        // if (!config->_const_op_on){
        //     // hardware ablation study
        //     // models extra clock cycles assuming hardware does not support const op
        //     if (config->_tile_interconnect == "htree")
        //         clocks = hTreeTile::getCycles(req, config) + req.precision_list[1].bits(); // tree height + precition
        //     else if (config->_tile_interconnect == "bus")
        //         clocks = 1 + req.precision_list[1].bits(); // only need 1 cycle to drive the bus, then precition cycles to store in cram rows.
        ...
        //         clocks = req.precision_list[1].bits(); // only need precition cycles to store in cram rows.
        // }
        //actual cycles to do in array computation
        {
    ...
    else if (op == "dp_cram_rf")
    {
        // if(!config->_const_op_on){
        //     //hardware ablation study
        //     //models extra clock cycles assuming const op is off
        //     if(config->_tile_interconnect == "htree")
        //         clocks = hTreeTile::getCycles(req, config) + req.precision_list[2].bits() + req.precision_list[3].bits();//tree height + precition of rf1 + precition of rf2
        //     else if(config->_tile_interconnect == "bus")
        //         clocks = 1 + req.precision_list[2].bits() + req.precision_list[3].bits();//only need 1 cycle to drive the bus, then some cycles to store in cram rows.
        //     else if(config->_tile_interconnect == "ideal")
        //         clocks = req.precision_list[2].bits() + req.precision_list[3].bits();//only need precition cycles to store in cram rows.
