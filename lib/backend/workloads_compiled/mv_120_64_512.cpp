...
int32_t mv_120_64_512(System *sys) {
  ...
  for (int32_t x_outer_outer = 0; x_outer_outer < 120; ++x_outer_outer) {
      ...
      for (int32_t k_inner = 0; k_inner < 64; ++k_inner) {
        {
          ...
          request.addOperand(sys->getAddress(x_outer_outer, 0, 64), 0, PrecisionT::Precision{0, 8, 0} /*CRAM*/);
        ...
        }
        {
          ...
          request.addOperand(sys->getAddress(x_outer_outer, 0, 144), 0, PrecisionT::Precision{0, 16, 0} /**/);
          request.addOperand(sys->getAddress(x_outer_outer, 0, 64), 0, PrecisionT::Precision{0, 8, 0} /*a.global[ramp(0, 1, 512)]*/);
        ...
        }
        {
          ...
          request.addOperand(sys->getAddress(x_outer_outer, 0, 144), 0, PrecisionT::Precision{0, 32, 0} /**/);
          request.addOperand(sys->getAddress(x_outer_outer, 0, 0), 0, PrecisionT::Precision{0, 32, 0} /*c.global.rf[ramp((k.outer*512), 1, 512)]*/);
          sys->sendRequest(request);
        }
      }
    }
    void* _7 = (void*) "c.global[ramp(0, 1, 512)] = x512(0)/*skip-init*/";
    #define max(a,b) ((a)>(b)?(a):(b))
    for (int32_t k_outer_v = 64, k_outer_v_cnt = 1; k_outer_v >= 1; k_outer_v -= max(k_outer_v / 2, 1), ++k_outer_v_cnt) {
    #undef max
      {
        Request request(Request::Type::BlockSend_Receive);
        request.addOperand(sys->getAddress(x_outer_outer, 0, 80), 0, PrecisionT::Precision{0, 32, 0} /*c.global[ramp(0, 1, 512)]*/);
        request.addOperand(sys->getAddress(x_outer_outer, 0, 0), 0, PrecisionT::Precision{0, 32, 0} /*c.global.rf[ramp((k.outer.v*512), 1, 512)]*/);
        sys->sendRequest(request);
      }
      {
        Request request(Request::Type::RowAdd);
        request.addOperand(sys->getAddress(x_outer_outer, 0, 80), 0, PrecisionT::Precision{0, 32, 0} /**/);
        request.addOperand(sys->getAddress(x_outer_outer, 0, 80), 0, PrecisionT::Precision{0, 32, 0} /*c.global[ramp(0, 1, 512)]*/);
        request.addOperand(sys->getAddress(x_outer_outer, 0, 0), 0, PrecisionT::Precision{0, 32, 0} /*c.global.rf[ramp((k.outer.v*512), 1, 512)]*/);
        sys->sendRequest(request);
      }
    }
    {
      ...
      request.addOperand(sys->getAddress(x_outer_outer, 0, 80), 0, PrecisionT::Precision{0, 32, 0} /*CRAM*/);
      sys->sendRequest(request);
    }
  }
  // freed c_global
