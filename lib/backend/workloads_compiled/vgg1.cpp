  ...
  for (int32_t ax1_outer = 0; ax1_outer < 56; ++ax1_outer) {
    // cram-array axis
    {
      ...
      // cram-array axis
      {
            ...
            for (int32_t ry1 = 0; ry1 < 3; ++ry1) {
              for (int32_t rx1 = 0; rx1 < 3; ++rx1) {
                ...
                }
                {
                  ...
                  request.addOperand(sys->getAddress(ax1_outer, 0, 0), 0, PrecisionT::Precision{1, 23, 8} /*DRAM*/);
                  request.addOperand(sys->getAddress(ax1_outer, 0, -1), 0, PrecisionT::Precision{1, 23, 8} /*CRAM*/);
                  sys->sendRequest(request);
                }
              }
            }
        ...
        void* _9 = (void*) "tensor[ramp((((ax1.outer*14336) + (ax1.inner*7168)) + (ax2*64)), 1, 64)] = x64(-3.40282e+38f)/*skip-init*/";
        for (int32_t rv0 = 0; rv0 < 2; ++rv0) {
          for (int32_t rv1 = 0; rv1 < 2; ++rv1) {
            {
              Request request(Request::Type::RowAdd_CRAM_RF);
              request.addOperand(sys->getAddress(ax1_outer, 0, 132), 0, PrecisionT::Precision{1, 23, 8} /**/);
              request.addOperand(sys->getAddress(ax1_outer, 0, 0), 0, PrecisionT::Precision{1, 23, 8} /*Conv2dOutput[ramp(((rv0*128) + (rv1*64)), 1, 64)]*/);
            ...
            }
            {
              Request request(Request::Type::RowLoad);
              request.addOperand(sys->getAddress(ax1_outer, 0, 0), 0, PrecisionT::Precision{1, 23, 8} /*DRAM*/);
              request.addOperand(sys->getAddress(ax1_outer, 0, 128), 0, PrecisionT::Precision{1, 23, 8} /*tensor[ramp((((ax1.outer*14336) + (ax1.inner*7168)) + (ax2*64)), 1, 64)]*/);
              sys->sendRequest(request);
            }
            {
              Request request(Request::Type::RowAdd);
            ...
            }
            {
              Request request(Request::Type::RowStore);
              request.addOperand(sys->getAddress(ax1_outer, 0, 0), 0, PrecisionT::Precision{1, 23, 8} /*DRAM*/);
              request.addOperand(sys->getAddress(ax1_outer, 0, 132), 0, PrecisionT::Precision{1, 23, 8} /*CRAM*/);
              sys->sendRequest(request);
            }
          }
        }
