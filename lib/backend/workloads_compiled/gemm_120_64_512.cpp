...
int32_t gemm_120_64_512(System *sys) {
  ...
  // int32_t compute_global[2048], 2560
  for (int32_t xo_outer = 0; xo_outer < 120; ++xo_outer) {
    for (int32_t y_inner = 0; y_inner < 32; ++y_inner) {
      // cram-array axis
      {
        int32_t rv_outer = 0;
        void* _2 = (void*) "compute.global.rf[ramp((rv.outer*512), 1, 512)] = x512(0)/*skip-init*/";
        for (int32_t rv_inner = 0; rv_inner < 32; ++rv_inner) {
          {
            Request request(Request::Type::RowLoad);
            request.addOperand(sys->getAddress(xo_outer, 0, 64), 0, PrecisionT::Precision{0, 8, 0} /*CRAM*/);
          ...
          }
          {
            ...
            request.addOperand(sys->getAddress(xo_outer, 0, 144), 0, PrecisionT::Precision{0, 16, 0} /**/);
            request.addOperand(sys->getAddress(xo_outer, 0, 64), 0, PrecisionT::Precision{0, 8, 0} /*a.global[ramp(0, 1, 512)]*/);
          ...
          }
          {
            ...
            request.addOperand(sys->getAddress(xo_outer, 0, 144), 0, PrecisionT::Precision{0, 32, 0} /**/);
            request.addOperand(sys->getAddress(xo_outer, 0, 0), 0, PrecisionT::Precision{0, 32, 0} /*compute.global.rf[ramp((rv.outer*512), 1, 512)]*/);
            sys->sendRequest(request);
          }
        }
      }
      void* _9 = (void*) "compute.global[ramp(0, 1, 512)] = x512(0)/*skip-init*/";
      #define max(a,b) ((a)>(b)?(a):(b))
      for (int32_t rv_outer_v = 64, rv_outer_v_cnt = 1; rv_outer_v >= 1; rv_outer_v -= max(rv_outer_v / 2, 1), ++rv_outer_v_cnt) {
      #undef max
        {
          Request request(Request::Type::BlockSend_Receive);
          request.addOperand(sys->getAddress(xo_outer, 0, 80), 0, PrecisionT::Precision{0, 32, 0} /*compute.global[ramp(0, 1, 512)]*/);
          request.addOperand(sys->getAddress(xo_outer, 0, 0), 0, PrecisionT::Precision{0, 32, 0} /*compute.global.rf[ramp((rv.outer.v*512), 1, 512)]*/);
          sys->sendRequest(request);
        }
        {
          Request request(Request::Type::RowAdd);
          request.addOperand(sys->getAddress(xo_outer, 0, 80), 0, PrecisionT::Precision{0, 32, 0} /**/);
          request.addOperand(sys->getAddress(xo_outer, 0, 80), 0, PrecisionT::Precision{0, 32, 0} /*compute.global[ramp(0, 1, 512)]*/);
          request.addOperand(sys->getAddress(xo_outer, 0, 0), 0, PrecisionT::Precision{0, 32, 0} /*compute.global.rf[ramp((rv.outer.v*512), 1, 512)]*/);
          sys->sendRequest(request);
        }
      }
      {
        ...
        request.addOperand(sys->getAddress(xo_outer, 0, 80), 0, PrecisionT::Precision{0, 32, 0} /*CRAM*/);
        sys->sendRequest(request);
      }
    }
  }
