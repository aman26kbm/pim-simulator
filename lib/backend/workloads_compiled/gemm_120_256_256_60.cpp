...
int32_t gemm_120_256_256_60(System *sys) {
  void* _1 = nullptr;
    ...
    // cram-array axis
    {
          ...
          for (int32_t y_c = 0; y_c < 8; ++y_c) {
            {
              ...
              request.addOperand(sys->getAddress(xo_outer, 0, 264), 262144, PrecisionT::Precision{0, 8, 0} /**/);
              request.addOperand(sys->getAddress(xo_outer, 0, 256), 65536, PrecisionT::Precision{0, 4, 0} /*a[ramp((((xo.outer*524288) + (rv.outer*2048)) + (rv.inner*256)), 1, 256)]*/);
              request.addOperand(sys->getAddress(xo_outer, 0, 0), 1024, PrecisionT::Precision{0, 4, 0} /*b[((((y.inner.outer*16384) + (y.c*2048)) + (rv.outer*8)) + rv.inner)]*/);
              sys->sendRequest(request);
            }
            {
              Request request(Request::Type::RowAdd);
              request.addOperand(sys->getAddress(xo_outer, 0, 0), 262144, PrecisionT::Precision{0, 16, 0} /**/);
              request.addOperand(sys->getAddress(xo_outer, 0, 264), 262144, PrecisionT::Precision{0, 16, 0} /**/);
      ...
      #undef max
        for (int32_t ax1 = 0; ax1 < 8; ++ax1) {
          ...
          {
            Request request(Request::Type::RowAdd);
            request.addOperand(sys->getAddress(xo_outer, 0, 128), 1024, PrecisionT::Precision{0, 16, 0} /**/);
            request.addOperand(sys->getAddress(xo_outer, 0, 128), 256, PrecisionT::Precision{0, 16, 0} /*compute.global[ramp((ax1*256), 1, 256)]*/);
            request.addOperand(sys->getAddress(xo_outer, 0, 0), 256, PrecisionT::Precision{0, 16, 0} /*compute.global.rf[ramp(((rv.outer.v*2048) + (ax1*256)), 1, 256)]*/);
            sys->sendRequest(request);
          }
...
static __attribute__((unused)) Registry::Entry &_gemm_120_256_256_60__ = pimsim::registerFunc("gemm_120_256_256_60", gemm_120_256_256_60);
