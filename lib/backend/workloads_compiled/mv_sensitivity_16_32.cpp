...
int32_t mv_sensitivity_compiled_16_32(System *sys) {
  void* _1 = nullptr;
  ...
  for (int32_t x_outer_outer = 0; x_outer_outer < 16; ++x_outer_outer) {
    for (int32_t x_outer_inner = 0; x_outer_inner < 8; ++x_outer_inner) {
      // cram-array axis
      {
        ...
        for (int32_t k_inner = 0, acc_offset_k_inner = 1, precision_cnt_k_inner; k_inner < 128; ++k_inner) {
          {
      ...
      for (int32_t k_outer_v = 32, k_outer_v_cnt = 1; k_outer_v >= 1; k_outer_v -= max(k_outer_v / 2, 1), ++k_outer_v_cnt) {
      #undef max
        {
          Request request(Request::Type::RowStore);
          request.type = Request::Type::RowAdd;
...
static __attribute__((unused)) Registry::Entry &__mv_sensitivity_compiled_16_32__ = pimsim::registerFunc("mv_sensitivity_compiled_16_32", mv_sensitivity_compiled_16_32);
