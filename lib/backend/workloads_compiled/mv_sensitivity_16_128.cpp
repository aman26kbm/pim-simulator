...
int32_t mv_sensitivity_compiled_16_128(System *sys) {
  void* _1 = nullptr;
  ...
  for (int32_t x_outer_outer = 0; x_outer_outer < 16; ++x_outer_outer) {
    for (int32_t x_outer_inner = 0; x_outer_inner < 8; ++x_outer_inner) {
      // cram-array axis
      {
        ...
        for (int32_t k_inner = 0, acc_offset_k_inner = 1, precision_cnt_k_inner; k_inner < 32; ++k_inner) {
          {
...
static __attribute__((unused)) Registry::Entry &__mv_sensitivity_compiled_16_128__ = pimsim::registerFunc("mv_sensitivity_compiled_16_128", mv_sensitivity_compiled_16_128);
