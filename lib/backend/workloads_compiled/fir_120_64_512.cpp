        ...
        for (int32_t rv_inner = 0; rv_inner < 32; ++rv_inner) {
          {
            ...
            request.addOperand(sys->getAddress(x_outer, 0, 32), 65536, PrecisionT::Precision{0, 16, 0} /*Unaligned Shift*/);
            request.addOperand(sys->getAddress(x_outer, 0, 32), 65536, PrecisionT::Precision{0, 16, 0} /*Unaligned Shift*/);
            sys->sendRequest(request);
          }
          {
            Request request(Request::Type::RowMul_CRAM_RF);
            request.addOperand(sys->getAddress(x_outer, 0, 64), 65536, PrecisionT::Precision{0, 16, 0} /**/);
            request.addOperand(sys->getAddress(x_outer, 0, 32), 65536, PrecisionT::Precision{0, 16, 0} /*a[ramp(((((x.outer*65536) + (x.inner.outer.outer*32768)) + (x.inner.outer.inner*512)) + rv.inner), 1, 512)]*/);
          ...
          }
          {
            ...
            request.addOperand(sys->getAddress(x_outer, 0, 0), 65536, PrecisionT::Precision{0, 16, 0} /*c.global[ramp(0, 1, 512)]*/);
            request.addOperand(sys->getAddress(x_outer, 0, 64), 65536, PrecisionT::Precision{0, 16, 0} /**/);
            sys->sendRequest(request);
          }
        }
