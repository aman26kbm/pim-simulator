      ...
      for (int32_t rv_v_outer = 0; rv_v_outer < 4; ++rv_v_outer) {
        // cram-array axis
        {
          ...
          }
          {
            ...
            request.addOperand(sys->getAddress(x_outer, 0, 64), 0, PrecisionT::Precision{0, 16, 0} /*a.global[ramp(0, 1, 512)]*/);
            request.addOperand(x_outer * 32, 0, PrecisionT::Precision{0, 16, 0} /*b[((rv.v.outer*64) + rv)]*/);
            sys->sendRequest(request);
          }
        }
        ...
        #undef max
          {
            ...
            request.addOperand(sys->getAddress(x_outer, 0, 0), 0, PrecisionT::Precision{0, 16, 0} /*c.global[ramp(0, 1, 512)]*/);
            request.addOperand(sys->getAddress(x_outer, 0, 32), 0, PrecisionT::Precision{0, 16, 0} /*c.global.rf[ramp((rv.v.inner*512), 1, 512)]*/);
            sys->sendRequest(request);
          }
          {
