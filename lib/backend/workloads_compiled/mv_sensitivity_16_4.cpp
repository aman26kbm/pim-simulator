...
int32_t mv_sensitivity_compiled_16_4(System *sys) {
  void* _1 = nullptr;
  // int32_t c_global_rf[1024], 0
  // int8_t a_global[256], 1024
  // int32_t c_global[1024], 1280
  // cram-tile axis
  for (int32_t x_outer_outer = 0; x_outer_outer < 16; ++x_outer_outer) {
    for (int32_t x_outer_inner = 0; x_outer_inner < 8; ++x_outer_inner) {
      // cram-array axis
      {
        int32_t k_outer = 0;
        {
        ...
        for (int32_t k_inner = 0, acc_offset_k_inner = 1, precision_cnt_k_inner; k_inner < 1024; ++k_inner) {
          {
      ...
      for (int32_t k_outer_v = 4, k_outer_v_cnt = 1; k_outer_v >= 1; k_outer_v -= max(k_outer_v / 2, 1), ++k_outer_v_cnt) {
      #undef max
        {
          Request request(Request::Type::RowStore);
          request.type = Request::Type::RowAdd;
          ...
          int32_t _5 = k_outer_v * 256;
          {
            ...
            request.addOperand(sys->getAddress(x_outer_outer/*tile no*/, 0/*block-id*/, ((_5) * 4/*bytes*/) / 32/*row-number*/ + 0/*cram buffer*/), 0, PrecisionT::Precision{false, 24 + k_outer_v_cnt, 0});
            request.addOperand(sys->getAddress(x_outer_outer/*tile no*/, 0/*block-id*/, ((_5) * 4/*bytes*/) / 32/*row-number*/ + 0/*cram buffer*/), 0, PrecisionT::Precision{false, 24 + k_outer_v_cnt, 0});
            sys->sendRequest(request);
          }
          request.addOperand(sys->getAddress(x_outer_outer/*tile no*/, 0/*block-id*/, ((_5) * 4/*bytes*/) / 32/*row-number*/ + 0/*cram buffer*/), 0, PrecisionT::Precision{false, 24 + k_outer_v_cnt, 0});
...
static __attribute__((unused)) Registry::Entry &__mv_sensitivity_compiled_16_4__ = pimsim::registerFunc("mv_sensitivity_compiled_16_4", mv_sensitivity_compiled_16_4);
