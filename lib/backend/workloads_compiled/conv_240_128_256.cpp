...
int32_t conv_240_128_256(System *sys) {
  void* _1 = nullptr;
  // int32_t Conv2dOutput_rf[1024], 0
  // int32_t Conv2dOutput_repl_global[1024], 1024
  for (int32_t ax0_outer = 0; ax0_outer < 4; ++ax0_outer) {
    for (int32_t ax0_inner_ax1_fused_ax2_fused = 0; ax0_inner_ax1_fused_ax2_fused < 224; ++ax0_inner_ax1_fused_ax2_fused) {
            ...
            for (int32_t rx = 0; rx < 3; ++rx) {
              for (int32_t rc_inner = 0; rc_inner < 4; ++rc_inner) {
                ...
                }
                {
                  ...
                  request.addOperand(sys->getAddress(ax0_inner_ax1_fused_ax2_fused, 0, 0), 0, PrecisionT::Precision{0, 8, 0} /*DRAM*/);
                  request.addOperand(sys->getAddress(ax0_inner_ax1_fused_ax2_fused, 0, 64), 0, PrecisionT::Precision{0, 8, 0} /*w[ramp((((((ry*786432) + (rx*262144)) + (rc.outer*2048)) + (rc.inner*512)) + (ax3.outer*256)), 1, 256)]*/);
                  sys->sendRequest(request);
                }
                {
                  ...
                  request.addOperand(sys->getAddress(ax0_inner_ax1_fused_ax2_fused, 0, 64), 0, PrecisionT::Precision{0, 8, 0} /*w[ramp((((((ry*786432) + (rx*262144)) + (rc.outer*2048)) + (rc.inner*512)) + (ax3.outer*256)), 1, 256)]*/);
                  request.addOperand(ax0_inner_ax1_fused_ax2_fused * 32, 0, PrecisionT::Precision{0, 8, 0} /*x[((((((((ax0.outer*184320) + ((ax0.inner.ax1.fused.ax2.fused/56)*46080)) + (((ax0.inner.ax1.fused.ax2.fused % 56)/7)*4608)) + (ry*4608)) + (rx*512)) + ((ax0.inner.ax1.fused.ax2.fused % 7)*512)) + (rc.outer*4)) + rc.inner)]*/);
                  sys->sendRequest(request);
                }
                {
...
static __attribute__((unused)) Registry::Entry &_conv_240_128_256__ = pimsim::registerFunc("conv_240_128_256", conv_240_128_256);
