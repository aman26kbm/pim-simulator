          ...
          for (int32_t rc_inner = 0; rc_inner < 2; ++rc_inner) {
            {
              ...
              request.addOperand(sys->getAddress(ax0_ax1_fused_ax2_fused, 0, 0), 128, PrecisionT::Precision{0, 8, 0} /*DRAM*/);
              request.addOperand(sys->getAddress(ax0_ax1_fused_ax2_fused, 0, 0), 128, PrecisionT::Precision{0, 8, 0} /*x[((((((((ax0.ax1.fused.ax2.fused/49)*20736) + (((ax0.ax1.fused.ax2.fused % 49)/7)*2304)) + (ry*2304)) + (rx*256)) + ((ax0.ax1.fused.ax2.fused % 7)*256)) + (rc.outer*2)) + rc.inner)]*/);
            ...
            }
            {
              ...
              request.addOperand(sys->getAddress(ax0_ax1_fused_ax2_fused, 0, 0), 128, PrecisionT::Precision{0, 8, 0} /*x[((((((((ax0.ax1.fused.ax2.fused/49)*20736) + (((ax0.ax1.fused.ax2.fused % 49)/7)*2304)) + (ry*2304)) + (rx*256)) + ((ax0.ax1.fused.ax2.fused % 7)*256)) + (rc.outer*2)) + rc.inner)]*/);
              request.addOperand(sys->getAddress(ax0_ax1_fused_ax2_fused, 0, 64), 32768, PrecisionT::Precision{0, 8, 0} /*w[ramp(((((ry*196608) + (rx*65536)) + (rc.outer*512)) + (rc.inner*256)), 1, 256)]*/);
              sys->sendRequest(request);
            }
            {
