...
int32_t fir_new(System* sys)
{
    ...
    request->addAddr(0, sys->_num_regs_per_rf, PrecisionT::INT4); //dst
    request->addAddr(sys->DRAM_ADDR, sys->_num_regs_per_rf, PrecisionT::INT4); //src
    ...
    for(int tile=0; tile<sys->_ntiles; tile++){
        ...
        request->addAddr(sys->getAddress(tile,0,12), 0, PrecisionT::INT16); //src
        requests.push_back(*request);
        ...
        for (int i=0; i< sys->_num_regs_per_rf; i++) {
            ...
            request->addAddr(sys->getAddress(tile,0,0), 0, PrecisionT::INT4); //src
            request->addAddr(sys->getAddress(tile,0,4), 0, PrecisionT::INT8); //dst
            requests.push_back(*request);

            //Now, add the product generated above into the accumulator rows
            request = new Request(Request::Type::RowAdd);
            request->addAddr(sys->getAddress(tile,0,4), 0, PrecisionT::INT16); //src
            request->addAddr(sys->getAddress(tile,0,12), 0, PrecisionT::INT16); //dst
            requests.push_back(*request);
            ...
            request->addAddr(sys->getAddress(tile,0,0), 0, PrecisionT::INT4);
            requests.push_back(*request);
