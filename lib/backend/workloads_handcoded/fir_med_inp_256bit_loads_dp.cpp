...
int32_t fir_med_inp_256bit_loads_dp(System* sys)
{
    ...
    if ( (use_tiles*t + tile) < tiles_needed ) {
        ...
        for(int iter_load_rf=0; iter_load_rf<iter_load_rf_count; iter_load_rf++){
            ...
            for (int i=0; i < cfg->_num_regs_per_rf/2; i++) {
...
static __attribute__((unused)) Registry::Entry &__fir_med_inp_256bit_loads__ = pimsim::registerFunc("fir_med_inp_256bit_loads_dp", fir_med_inp_256bit_loads_dp);
