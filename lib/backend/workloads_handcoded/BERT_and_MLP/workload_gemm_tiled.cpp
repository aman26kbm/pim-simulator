...
int32_t gemm_tiled(System* sys, std::string param_file){
    //Parameters:
    int M = 256;
    int K = 256;
    int N = 256;
    ...
    func_gemm_tiled(M, K, N, precision_input, precision_multiply, precision_accumulate, precision_result, requests, sys);
...
static __attribute__((unused)) Registry::Entry &__gemm_tiled__ = pimsim::registerFunc("gemm_tiled", gemm_tiled);
