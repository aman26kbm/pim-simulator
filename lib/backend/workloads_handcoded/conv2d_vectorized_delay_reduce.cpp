...
int32_t conv2d_vectorized_delay_reduce(System* sys, std::string param_file)
{
                            ...
                            for(int c_=0; c_<ceil(C/(float)numArrayPerTile); c_++){//serial, for reduction
                                ...
                                for(int r=0; r<R; r++){//serial
                                    for(int s=0; s<S; s++){//serial
                                        ...
                                        int W_offset = m_*C*R*S*numColPerArray + c_*R*S*numArrayPerTile*numColPerArray + (r*S + s)*numArrayPerTile*numColPerArray ;
                                        sys->map_vec_to_tile(requests, tile, "W", W_offset, numArrayPerTile*numColPerArray, precision_input.bits(), numColPerArray*numArrayPerTile, 0, 0, 0, 0);
                                        
                                        //load I matrix
                                        //offset of the original I matrix.
                                        int I_offset_initial = n*H*W*C + (e_*H_Yp+e__+r)*W*C + (f_*W_Yp+f__+s)*C + c_*numArrayPerTile;
                                        ...
                                        int I_offset = n*H*W*C + (e_*H_Yp+e__+r)*W*C + (f_*W_Yp+f__+s)*C + c_*numArrayPerTile;
                                        sys->map_vec_to_tile(requests, tile, "I", I_offset,numColPerArray, precision_input.bits(), numArrayPerTile, 0, 1, 0, 1);
...
static __attribute__((unused)) Registry::Entry &__conv2d_vectorized_delay_reduce__ = pimsim::registerFunc("conv2d_vectorized_delay_reduce", conv2d_vectorized_delay_reduce);
