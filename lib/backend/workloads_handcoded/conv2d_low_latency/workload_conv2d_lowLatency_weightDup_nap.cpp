...
int32_t conv2d_lowLatency_weightDup_nap(System* sys, std::string param_file)
{
    ...
    //start compute
    conv2d_low_latency_compute_weight_dup_nap(conv_layer_params, precision_input, precision_multiply, precision_accumulate, requests, sys);
    //store output
...
static __attribute__((unused)) Registry::Entry &__conv2d_lowLatency_weightDup_nap__ = pimsim::registerFunc("conv2d_lowLatency_weightDup_nap", conv2d_lowLatency_weightDup_nap);
