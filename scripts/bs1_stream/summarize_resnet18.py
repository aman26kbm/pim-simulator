...
for filename in sorted(glob.glob(os.path.join(path, '*.log'))):
   with open(os.path.join(os.getcwd(), filename), 'r') as f: # open in readonly mode
        ...
        if workload == "param_conv2d_all":
            layer_data[2] = cycles
        elif workload == "param_conv2d_weight_load":
            layer_data[3] = cycles
        elif workload == "param_conv2d_input_load":
            layer_data[4] = cycles
        elif workload == "param_conv2d_compute":
            layer_data[5] = cycles
        elif workload == "param_conv2d_store":
            layer_data[6] = cycles
        else:
