...
axes.append(fig.add_subplot(gs[0:4,0:2]))
axes.append(fig.add_subplot(gs[5:11,0:2]))
...
def plot_bars(ax, data, labels, idx, title, bbox, ylims, color):
    ...
    ax.set_xticklabels(['vecadd', 'fir', 'gemv', 'gemm', 'conv2d'], rotation=90, fontsize=18)
    ...
    if idx == 0:
        ax.legend(handlelength=1, frameon=False, ncol=3, handletextpad=0.5, labelspacing=0.2, loc='upper center', bbox_to_anchor=bbox, fontsize=15)
    if idx == 1: 
        ax.legend(handlelength=1, frameon=False, ncol=5, columnspacing=1.5, handletextpad=0.2, labelspacing=0.2, loc='upper center',bbox_to_anchor=bbox, fontsize=15)
...
labels = ['half',
          'main',
          'double']
gms += plot_bars(axes[0], bars, labels, 0, '(a) Data Size Sens.', (0.5, 1), (0, 2.5), ['w', binary(0.45), binary(0.7)])
...
labels = ['i4',
          ...
          'i7',
          'i8']
gms += plot_bars(axes[1], bars, labels, 1, '(b) Precision Sens.', (0.5, 1), (0, 3), ['w', binary(0.45), binary(0.55), binary(0.65), binary(0.75)])



print(gms)

axes[0].set_ylabel('Relative Exec. Time \n Normalized to Main',fontsize=18)
axes[1].set_ylabel('Relative Exec. Time \n Normalized to i4',fontsize=18)


fig.subplots_adjust(top=0.93, bottom=0.10, left=0.1, right=0.53, wspace=0.02, hspace=1)
plot.show()
