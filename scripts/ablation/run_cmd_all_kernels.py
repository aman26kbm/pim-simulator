...
for (task, log_file) in runs:
    ...
    subprocess.run([EXEC_PATH, "-c", CFG_PATH, "-m", task, "-l", f"{log_file}.log"])
...
for (task, log_file) in runs_conv:
    ...
    for ablation in ablations:
        ...
        if ablation == "shuffleOff" or ablation == "everythingOff":
            subprocess.run([EXEC_PATH, "-c", f"{ROOT_PATH}/configs/DMesh12x10_TileSize256_CramSize256x256_{ablation}.cfg", "-m", f"{task}_shuffleOff","-p", PARAM_PATH, "-l", f"{log_file}_{ablation}.log"])
        else:
